...
export async function addShapeWithTextElement(
    ...
    try {
    // アイコンが存在する場合は画像を追加
    if (awsServiceName && imageData) {
        console.log(`[PPTX Convert] Added AWS icon Image for "${awsServiceName}" at (${x}, ${y}) with base64 data`)
        ...
        console.log(`[PPTX Convert] Added AWS icon for "${awsServiceName}" at (${x}, ${y}) with base64 data`)
